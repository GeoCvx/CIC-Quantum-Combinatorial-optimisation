...
def qp_objective_value(problem: ProblemData, y: np.ndarray) -> float:
    """
    固定 x 后，连续子问题的目标值（不含固定成本）：
        sum_i [ (p_i - alpha_i) y_i - beta_i y_i^2 ]
    """
    return float(np.sum(problem.c * y - problem.beta * y * y))


def total_objective_value(problem: ProblemData, x: np.ndarray, y: np.ndarray) -> float:
    """
    原问题总目标值：
        sum_i [ (p_i - alpha_i) y_i - beta_i y_i^2 - f_i x_i ]
    """
    return float(np.sum(problem.c * y - problem.beta * y * y - problem.fixed_cost * x))